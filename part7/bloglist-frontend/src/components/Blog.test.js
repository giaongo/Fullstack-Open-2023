...
test('renders blog content without url and likes', () => {
  ...
  expect(div).toHaveTextContent(
    'NTS: Reliable Device Testing at Scale - Netflix',
  );
  expect(div).not.toHaveTextContent('1');
  expect(div).not.toHaveTextContent(
    'https://medium.com/@free-thinker/forget-chatgpt-you-will-not-regret-using-these-ai-tools-in-2023-7c2a39cc4901',
  );
});
...
describe('Testing button togglable', () => {
  ...
  beforeEach(() => {
    ...
    container = render(
      <Blog blog={blog} increaseLikeNumber={likeMockHandler} />,
    ).container;
  });
